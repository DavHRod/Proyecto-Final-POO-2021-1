...
def sql_connection():
    try:
        con = sqlite3.connect('PruebaLocochona2.db')
        return con
    except Error:
        print(Error)
...
def sql_table(con):
    ...
    #cursorObj.execute("CREATE TABLE Porfa(Ojala integer PRIMARY KEY, Funcione text)")
    a=int(input())
    b=input()
    c=(a,b)
    cursorObj.execute('''INSERT INTO Porfa(Ojala,Funcione) VALUES(?,?)''',c)
    con.commit()
#Funcion Menu
"""
def Menu():
